...
class View:
    ...
    def plot(self):
        if self.dimension == 'single':
            ...
            return dcc.Graph(className='graph',id={'type': 'graph','index': self.index}, figure=getattr(figs, self.chain)(self.quantity, self.dataset, self.colorbar, self.wigner, self.proton, self.neutron, self.nucleon, self.range, self.uncertainty))
